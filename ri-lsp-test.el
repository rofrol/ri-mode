...
(ert-deftest ri-lsp-test-pick-menu-opens-legend-and-transient-map ()
  (let (shown transient)
    ...
    (should-not (cadr transient))
    (should (functionp (caddr transient)))))

(ert-deftest ri-lsp-test-picker-first-query-keys-bypass-menu-maps ()
  (let ((buffer (generate-new-buffer " *ri-picker-query-test*"))
        (real-set-transient-map (symbol-function 'set-transient-map))
        (ri--menu-state nil)
        transient-exits
        close
        (open-count 0))
            ...
            (cl-letf
                (((symbol-function 'set-transient-map)
                  (lambda (&rest arguments)
                    (let ((exit
                           (apply real-set-transient-map arguments)))
                      (push exit transient-exits)
                      exit)))
                 ((symbol-function 'keymap-legend-show) #'ignore)
                 ((symbol-function 'keymap-legend-hide) #'ignore)
                 ((symbol-function 'ri--hide-frame) #'ignore)
                 ((symbol-function 'ri-pick-open-buffers)
                  #'open-picker)
                 ((symbol-function 'ri-pick-open-files)
                  #'open-picker)
                 ((symbol-function 'ri-lsp-pick-document-symbols)
                  #'open-picker)
                 ((symbol-function 'ri-lsp-pick-workspace-symbols)
                  #'open-picker))
              (ri-space-menu)
              (execute-kbd-macro (kbd "k d s d f S x"))
              (should (= open-count 1))
              (should (equal (buffer-string) "sdfSx"))
...
(ert-deftest ri-lsp-test-picker-handoff-hides-legend-before-opening ()
  (let ((ri--menu-state 'pick)
        events
        close)
    (cl-letf (((symbol-function 'keymap-legend-hide)
               (lambda ()
