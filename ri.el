...
(defvar ri--normal-help-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "k" '(menu-item "v" sr-nav-down))
    (define-key map "u" '(menu-item "<" sr-nav-prev))
    (define-key map "o" '(menu-item ">" sr-nav-next))
    (define-key map "y" '(menu-item "|<" sr-nav-first))
    (define-key map "p" '(menu-item ">|" sr-nav-last))
    ...
    (define-key map "s" '(menu-item "WORD" sr-set-word-mode))
    (define-key map "S" '(menu-item "WORD*" sr-set-word-star-mode))
    (define-key map (kbd "M-s") '(menu-item "WORD+" sr-set-word-plus-mode))
    (define-key map "w" '(menu-item "SUBWORD" sr-set-subword-mode))
    (define-key map "v" '(menu-item "≡ Paste" ri-space-paste))
...
(defun ri-enable ()
  ...
  (define-key mini-modal-map "s" #'sr-set-word-mode)
  (define-key mini-modal-map "S" #'sr-set-word-star-mode)
  (define-key mini-modal-map (kbd "M-s") #'sr-set-word-plus-mode)
  (define-key mini-modal-map "w" #'sr-set-subword-mode)
