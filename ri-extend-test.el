...
(ert-deftest ri-extend-test-node-source-line-navigation-preserves-edges ()
  (ri-extend-test--with-buffer "(top 1)\n(bottom 2)"
    (unless (and (treesit-available-p)
                 (treesit-language-available-p 'elisp))
      (ert-skip "Elisp tree-sitter grammar unavailable"))
    (emacs-lisp-mode)
    (treesit-parser-create 'elisp)
    (let (top-bounds bottom-bounds)
      (search-forward "top")
      (setq sr-submode 'line)
      (ri-extend-set-node-mode)
      (setq top-bounds (sr--get-current-unit-bounds))
      (search-forward "bottom")
      (setq sr-submode 'line)
      (ri-extend-set-node-mode)
      (setq bottom-bounds (sr--get-current-unit-bounds))

      (ri-extend-nav-line-up)
      (should (eq sr-submode 'node))
      (should (equal (sr--get-current-unit-bounds) top-bounds))
      ...
      (ri-swap-cursor)
      (ri-extend-nav-line-up)
      (should (equal (ri--selection-bounds)
                     (cons (car top-bounds) (cdr bottom-bounds))))
      (should (= (point) (car top-bounds)))
      (should (eq (ri--selection-state-active-edge ri--selection) 'start))
      (ri--exit-extend)

      (goto-char (car top-bounds))
      (setq sr-submode 'line)
      (ri-extend-set-node-mode)
      (should (ri--enter-extend))
      ...
      (should (equal (ri--selection-bounds)
                     (cons (car top-bounds) (cdr bottom-bounds))))
      (should (= (point) (1- (cdr bottom-bounds))))
      (should (eq (ri--selection-state-active-edge ri--selection) 'end))
      (ri--exit-extend))))
