...
(ert-deftest ri-extend-test-char-bidirectional-expansion ()
  (ri-extend-test--with-buffer "abcdef"
    ...
    (ri-extend-nav-right)
    (ri-extend-nav-right)
    (should (equal (ri--selection-bounds) (cons 3 4)))
    (should (= (point) 3))
    (should (eq (ri--selection-state-active-edge ri--selection) 'start))
    (ri-extend-nav-right)
    (ri-extend-nav-right)
    ...
    (should (eq (ri--selection-state-active-edge ri--selection) 'end))
    (ri-extend-nav-left)
    (should (equal (ri--selection-bounds) (cons 2 4)))
    (should (= (point) 2))
    (should (eq (ri--selection-state-active-edge ri--selection) 'start))
    (ri--exit-extend)))
