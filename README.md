...
| `a`                  | Select `LINE`; hold with `i` / `j` / `k` / `l` for `CHAR` movement |
...
`i` / `j` / `k` / `l` to navigate by characters, or hold `s` with the same
keys to navigate by WORD+ units. Releasing a held `a` or `s` returns to the
